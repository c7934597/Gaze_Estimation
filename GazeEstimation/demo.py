...
class Demo:
    ...
    def _process_image(self, image) -> None:
        ...
        for face in faces:
            ...
            head_pitch, head_yaw, head_roll, distance = self._draw_head_pose(face)
            self._draw_landmarks(face)
            self._draw_face_template_model(face)
            gaze_pitch, gaze_yaw = self._draw_gaze_vector(face)
            self._display_normalized_image(face)

            cv2.putText(self.visualizer.image, "Head Yaw :" + str(round(head_yaw,2)), (int(face.bbox[0][0]), int(face.bbox[0][1]-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), thickness=2)
            cv2.putText(self.visualizer.image, "Head Pitch :" + str(round(head_pitch,2)), (int(face.bbox[0][0]), int(face.bbox[0][1]-30)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), thickness=2)
            cv2.putText(self.visualizer.image, "Gaze Yaw :" + str(round(gaze_yaw,2)), (int(face.bbox[0][0]), int(face.bbox[0][1]-50)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), thickness=2)
            cv2.putText(self.visualizer.image, "Gaze Pitch :" + str(round(gaze_pitch,2)), (int(face.bbox[0][0]), int(face.bbox[0][1]-70)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), thickness=2)
